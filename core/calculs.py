def calculer_sommes_par_chapitre(df, annees):
    ...
    return sommes, report_a_nouveau
...
def calcul_autofinancement(df):
    ...
    return {
        ...
        "Epargne_brute": f"{epargne_brute:,.2f} €",
        "Epargne_nette": f"{epargne_nette:,.2f} €",
        "Report_n1": f"{report_n1:,.2f} €",
        "Disponibilite": f"{(epargne_nette + report_n1):,.2f} €"
    }
